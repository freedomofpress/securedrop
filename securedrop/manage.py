...
def add_admin(args):
    return _add_user(args, is_admin=True)
...
def _add_user(args, is_admin=False):
    context = app_context()
    context.push()
    if args.username:
    ...
    try:
        ...
        if args.password:
            ...
            user.pw_hash = user._scrypt_hash(args.password, user.pw_salt)
        if args.otp_secret:
            user.otp_secret = args.otp_secret
        db.session.add(user)
        db.session.commit()
        ...
        if not otp_secret and not args.otp_secret:
...
def get_args():
    ...
    set_add_admin_parser(subps, 'add-admin')
    set_add_admin_parser(subps, 'add_admin')
...
def set_add_admin_parser(subps, name):
    parser = subps.add_parser(name, help='Add an admin to the application.')
    parser.add_argument('--password', help='force password')
    parser.add_argument('--username', help='force username')
    parser.add_argument('--totp', action='store_true', help='force TOTP')
    parser.add_argument('--otp-secret', help='force OTP secret')
    parser.set_defaults(func=add_admin)
