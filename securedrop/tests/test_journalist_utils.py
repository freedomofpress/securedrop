...
def test_revoke_token_cleanup_does_not_delete_tokens_if_not_expired(
...
def test_revoke_token_cleanup_does_deletes_tokens_that_are_expired(
    journalist_app, test_journo, journalist_api_token, mocker
):
    with journalist_app.test_client() as app:
        resp = app.post(url_for("api.logout"), headers=get_api_headers(journalist_api_token))
        assert resp.status_code == 200
        ...
        mocker.patch(
            "journalist_app.admin.Journalist.validate_token_is_not_expired_or_invalid",
            return_value=None,
        )
        cleanup_expired_revoked_tokens()
