...
# Note: we use the `journalist_app` fixture because it creates the DB
def test_exception_handling_when_duplicate_username(journalist_app, config, mocker, capsys):
...
def test_were_there_submissions_today(source_app, config, app_storage):
    with source_app.app_context() as context:
        ...
        source = SourceFactory.create(
            db.session,
            app_storage,
            last_updated=datetime.datetime.utcnow() - datetime.timedelta(hours=24 * 2),
        )
