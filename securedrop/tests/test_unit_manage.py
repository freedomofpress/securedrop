...
class TestManagePy(unittest.TestCase):
    ...
    def test_get_test_module_dict_functional(self):
        ...
        for file in os.listdir(os.path.join(ABS_MODULE_DIR_PATH, "functional")):
            if fnmatch.fnmatch(file, 'test*.py'):
                self.assertIn(os.path.join(ABS_MODULE_DIR_PATH, 'functional',
                                           file),
                              output.values())
        # The test modules short names should not contain path, extension, or
        # other unecessary text
        for test in list(output):
            self.assertNotRegexpMatches(ABS_MODULE_DIR_PATH, test)
    ...
    def test_get_test_module_dict_unit(self):
        ...
        # All test modules should be present
        for file in os.listdir(ABS_MODULE_DIR_PATH):
            if fnmatch.fnmatch(file, 'test*.py'):
                self.assertIn(os.path.join(ABS_MODULE_DIR_PATH, file),
                              output.values())
        # The test modules short names should not contain path, extension, or
        # other unecessary text
        for test in list(output):
