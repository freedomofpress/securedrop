...
def test_alembic_head_matches_db_models(journalist_app,
    ...
    os.remove(journalist_config.DATABASE_FILE)
    subprocess.check_call(
        ['sqlite3', journalist_config.DATABASE_FILE, '.databases'])

    # Use the fixture to rewrite the config with the new URI
    conftest.alembic_config(journalist_config)

    # Upgrade to the latest
    upgrade(alembic_config, 'head')

    # Recreate the app to get a new SQLALCHEMY_DATABASE_URI
    app = create_app(journalist_config)
...
def test_alembic_migration_upgrade(alembic_config, migration):
...
@pytest.mark.parametrize('migration', ALL_MIGRATIONS)
def test_alembic_migration_downgrade(alembic_config, migration):
...
@pytest.mark.parametrize('migration', ALL_MIGRATIONS)
def test_upgrade_with_data(alembic_config, journalist_config, migration):
    migrations = list_migrations(alembic_config, migration)
    if len(migrations) == 1:
    ...
    # Load the test data
    upgrade_tester = mod.UpgradeTester(config=journalist_config)
...
@pytest.mark.parametrize('migration', ALL_MIGRATIONS)
def test_downgrade_with_data(alembic_config, journalist_config, migration):
    # Upgrade to the target
    upgrade(alembic_config, migration)
    ...
    # Load the test data
    downgrade_tester = mod.DowngradeTester(config=journalist_config)
