...
_DEFAULT_SECUREDROP_ROOT = Path(__file__).absolute().parent.parent.parent


# TODO(AD): This mirrors the field in an SDConfig; it could eventually replace it
@dataclass(frozen=True)
class SecureDropConfig:
    ...
    SECUREDROP_ROOT: str = str(_DEFAULT_SECUREDROP_ROOT)
    TRANSLATION_DIRS: Path = _DEFAULT_SECUREDROP_ROOT / "translations"
    SOURCE_TEMPLATES_DIR = str(_DEFAULT_SECUREDROP_ROOT / "source_templates")
    JOURNALIST_TEMPLATES_DIR = str(_DEFAULT_SECUREDROP_ROOT / "journalist_templates")
    WORD_LIST = str(_DEFAULT_SECUREDROP_ROOT / "wordlist")
    NOUNS = str(_DEFAULT_SECUREDROP_ROOT / "dictionaries" / "nouns.txt")
    ADJECTIVES = str(_DEFAULT_SECUREDROP_ROOT / "dictionaries" / "adjectives.txt")
