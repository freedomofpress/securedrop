...
class TestSourceAppStaticPages:
    ...
    def test_notfound(self, locale, sd_servers_v2):
    ...
    @pytest.mark.parametrize("locale", list_locales())
    def test_static_pages(self, locale, sd_servers_v2):
        # Given a source user accessing the app from their browser
        locale_with_commas = locale.replace("_", "-")
        with get_web_driver(
            web_driver_type=WebDriverTypeEnum.TOR_BROWSER,
            ...
            tor_browser_web_driver.get(f"{sd_servers_v2.source_app_base_url}/use-tor")
            save_screenshot_and_html(tor_browser_web_driver, locale, "source-use_tor_browser")

            tor_browser_web_driver.get(f"{sd_servers_v2.source_app_base_url}/tor2web-warning")
            save_screenshot_and_html(tor_browser_web_driver, locale, "source-tor2web_warning")

            tor_browser_web_driver.get(f"{sd_servers_v2.source_app_base_url}/why-public-key")
