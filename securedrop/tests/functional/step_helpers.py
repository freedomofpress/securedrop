...
# screenshots is a decorator that records an image before and after the steps described in this file
def screenshots(f):
    def wrapper(*args):
        ...
        stack = [x for x in traceback.extract_stack() if '/tests/functional' in x[0]]
        path = '-'.join([stack[0][0].split('/')[-1], class_name] + [x[2] for x in stack if x[2] is not 'wrapper'])
