...
function enhance_ui() {
  ...
  $('div#filter-container').html('<input id="filter" type="text" placeholder="' + get_string("filter_by_codename_placeholder") + '" autofocus >');

  // Add the "select {all,none}" buttons
  $('div#select-container').html('<span id="select_all" class="select"><i class="fa fa-check-square-o"></i>' + get_string("select_all") + '</span> <span id="select_none" class="select"><i class="fa fa-square-o"></i>' +  get_string("select_none") + '</span>');
...
$(function () {
  ...
  $("#delete_collection").submit(function () {
    return confirm(get_string("collection_delete_confirm"));
  });
  ...
  $("#delete_collections").click(function () {
    ...
    if (checked.length > 0) {
      return confirm(get_string("collection_multi_delete_confirm").supplant({ size: checked.length }));
    }
  ...
  $("#delete_selected").click(function () {
      ...
      if (checked.length > 0) {
          return confirmed = confirm(get_string('Are you sure you want to delete the {size} selected submissions?').supplant({ size: checked.length }));
      }
  ...
  $('button.delete-user').click(function(event) {
      ...
      return confirm(get_string("delete_user_confirm").supplant({ username: username }));
  });

  // Confirm before resetting two factor authentication on edit user page
  $('form#reset-two-factor').submit(function(event) {
      return confirm(get_string("reset_user_mfa_confirm").supplant({ username: username }));
  });
