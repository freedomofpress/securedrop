...
def submit():
    ...
    if first_submission:
        flash(
            _("Thanks for submitting something to SecureDrop! Please check back later for replies."),
            "notification")
    else:
        if msg:
            flash(_("Thanks! We received your message."), "notification")
        if fh:
            flash(_("Thanks! We received your document \"{filename}\".").format(filename=fh.filename or '[unnamed]'), "notification")
...
def login():
    if request.method == 'POST':
        ...
        if valid_codename(codename):
            ...
            app.logger.info(
                    _("Login failed for invalid codename {codename}").format(codename=codename))
