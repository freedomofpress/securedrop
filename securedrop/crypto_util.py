...
class AES_then_HMAC:
    ...
    def gen_keys_string(cls):
        ...
        return keys.encode("base64").replace("\n", "")
    ...
    def extract_keys(cls, keys_string):
        ...
        enc_key, auth_key = keys[:-cls.digest_size], keys[-cls.digest_size:]
    ...
    def encrypt_then_mac(cls, keys_string, plaintext):
        ...
        ciphertext = AES.new(enc_key, cls.mode, counter=ctr).encrypt(plaintext)
        mac = hmac.new(auth_key, ciphertext.encode("utf-8"), cls.digestmod).digest()
        return ciphertext + mac

    @classmethod
    def authenticate_then_decrypt(cls, keys_string, data):
        ...
        :raises: An :exc:`AuthenticationError` when the MAC cannot be 
                 authenticated.
        ...
        if hmac.new(auth_key, ciphertext.encode("utf-8"), cls.digestmod).digest() != mac:
            raise BadSignature("Signature authentication failed!")
        ctr = Counter.new(cls.iv)
        return AES.new(enc_key, cls.mode, counter=ctr).decrypt(ciphertext)
