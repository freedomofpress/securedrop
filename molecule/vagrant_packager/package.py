...
class LibVirtPackager(object):
    ...
    def _get_virsh_xml(self, cmd: str) -> ET.Element:
    ...
    def image_rebase(self, img_location: str):
        ...
        if self.default_image_location() in img_location:
            raise UserWarning("To prevent catastrophy, will not"
                    " run on image in the default pool. Copy it first to "
                    "someplace else")
    ...
    def image_store_path(self) -> str:
        """ Get location of VM's first storage disk file """
        vm_xml = self.vm_xml()
        return vm_xml.findall('./devices/disk/source')[0].attrib['file']

    def image_sparsify(self, src: str, dest: str, tmp_dir: str, inplace: bool):
        """ Remove blank-space from the image. Note that setting inplace to
            false will get you better bang for your buck but it can be extremely
    ...
    def sysprep(self, img_location: str):
        ...
        sysprep_cmd = ("virt-sysprep --no-logfile --operations "
                      "defaults,-ssh-userdir,-ssh-hostkeys -a "+img_location)
        subprocess.check_call(sysprep_cmd.split())

    def vagrant_metadata(self, img_location: str) -> dict:
