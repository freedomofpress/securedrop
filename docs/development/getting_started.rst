...
.. note:: Tested on: Ubuntu 14.04 and Debian sid (May 6th)
...
latest stable ansible to a virtual environment using `virtualenvwrapper` or
`virtualenv`.
...
Using virtualenv (must be in SecureDrop base directory before running these
commands):
