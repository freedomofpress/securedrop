...
class TestJournalistMail(TestBase):
    ...
    def test_procmail(self, host):
        ...
        for (origin, destination, payload) in (
                ('journalist', 'journalist', 'ossec: output\n12345'),
                ('root', 'ossec', 'MYGREATPAYLOAD')):
            assert self.run(host, "postsuper -d ALL")
            assert self.run(
                host,
                "echo -e '{payload}' | "
                "mail -s 'abc' {origin}@localhost".format(
                    origin=origin,
                    payload=payload))
            assert self.wait_for_command(
                host,
    ...
    def test_journalist_mail_notification(self, host):
        ...
        def change_frequency(value):
            return """
            perl -p -e 's/frequency>\d+</frequency>{value}</ \
                        if(m|head -1 .*submissions_today.txt|..m|frequency>|)'
            """.format(value=value)

        with app.sudo():
            assert self.run(app, change_frequency('7'))
        ...
        with app.sudo():
            ...
            assert self.run(app, change_frequency('86400'))
