...
start_authd() {
  ...
  echo "authd is running ensure that after the App Server's OSSEC agent connects to disable authd"
}
...
check_status() {
  ...
  sleep 15
  if ! /var/ossec/bin/list_agents -c | grep "^app-$SOURCE_IP"; then
    echo "The App Server's agent is not active."
