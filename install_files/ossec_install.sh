...
if [ $ROLE = 'source' -o $ROLE = 'monitor' ]; then
...
start_agent_auth() {
  ...
  /var/ossec/bin/agent-auth -m $MONITOR_IP -p 1515 -A source
  catch_error $? "starting ossec agent auth"
}

kill_authd() {
  echo "Leaving authd running is a security risk."
  echo "Start the installation on source server."
  echo "After installation on the source server is complete"
...
check_status() {
  ...
  if ! /var/ossec/bin/list_agents -c | grep "^source-$SOURCE_IP"; then
    ...
    echo "Exiting with errors. The source server's ossec agent is not connected"
    exit 0
  fi
}
