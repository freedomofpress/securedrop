...
if [[ $WHAT == "ossec" ]]; then
    ...
    $DOCKER_BIN run --rm -v "$(pwd)":/src -e VARIANT=agent --entrypoint "/build-debs-ossec" fpf.local/sd-server-builder
    $DOCKER_BIN run --rm -v "$(pwd)":/src -e VARIANT=server --entrypoint "/build-debs-ossec" fpf.local/sd-server-builder
else
    $DOCKER_BIN run --rm -v "$(pwd)":/src --entrypoint "/build-debs-securedrop" fpf.local/sd-server-builder
fi
